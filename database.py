...
def init_player(username: str, class_name: str):
    ...
    def starter_weapon(player_id: int):
        ...
        class_id = result[0]
        c.execute("SELECT name FROM WHERE id = ?", (class_id,))
        class_name = c.fetchone()

        c.execute("""
            SELECT id, name, hit_mult, bonus_hp, bonus_hit, bonus_wisdom
            FROM weapons
            WHERE class_type = ?
        """,(class_name,))
        weapons = c.fetchall()
        ...
        if not weapons:
            print("No weapons found for this class!")
            return

        low_weapons = [w for w in weapons if w[2] <= 10]

        if not low_weapons:
            print("No low-stat weapons found, picking any weapon instead.")
            chosen_weapon = random.choice(weapons)
        else:
            chosen_weapon = random.choice(low_weapons)
        
        weapon_name = chosen_weapon[1]
        add_item(player_id, weapon_name, 1)
        print(f"{weapon_name} added to player {player_id}'s inventory!")
    conn.commit()
