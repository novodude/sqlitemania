...
def generate_gear(player_level: int, gear_type: str = "random"):
    ...
    if class_type == "The Executor":
        bonus_hp    = min(100, random.randint(20, 50) + level_bonus * 20)
        bonus_hit   = min(100, random.randint(5, 20) + level_bonus * 3)
        bonus_crit  = min(100, random.randint(0, 4) + level_bonus)
        hit_mult    = random.randint(1, 5)

    elif class_type == "The Trigger":
        bonus_hp    = min(100, random.randint(5, 25) + level_bonus * 12)
        bonus_hit   = min(100, random.randint(10, 30) + level_bonus * 4)
        bonus_crit  = min(100, random.randint(5, 15) + level_bonus * 2)
        hit_mult    = random.randint(1, 5)

    else:  # The Indexer
        bonus_hp    = min(100, random.randint(20, 100) + level_bonus * 5)
        bonus_hit   = min(100, random.randint(5, 20) + level_bonus * 2)
        bonus_crit  = min(100, random.randint(10, 30) + level_bonus * 4)
        hit_mult    = random.randint(1, 5)
    ...
    if gear_type == "weapon":
        ...
        c.execute(f"""
            INSERT INTO {table} (name, class_type, bonus_hp, bonus_hit, bonus_crit, element, found)
            VALUES (?, ?, ?, ?, ?, ?, 0)
...
def init_player(username: str, class_name: str):
    ...
    c.execute("""
        INSERT INTO player_stats (player_id, base_hp, max_hp, current_hp, original_base_hit, base_hit, base_crit)
        VALUES (?, ?, ?, ?, ?, ?, ?)
...
def bonus_calc(bonus_type: BonusType, player_id: int):
    ...
    # class bonus
    if data["class_type"] == user_class_type:
    ...
    # apply multiplier only for weapons
    if hit_mult is not None:
        c.execute("""
            UPDATE player_stats
            SET base_hit = original_base_hit * ?
            WHERE player_id = ?
        """, (hit_mult, player_id))
...
ENEMY_PROFILES = {
    "Corrupted Index": {"hp_mult": 1.3, "hit_mult": 0.8, "xp_mult": 1.0},  # tanky, slow
    "Null Pointer":    {"hp_mult": 0.8, "hit_mult": 1.4, "xp_mult": 1.1},  # glass cannon
    "Stack Overflow":  {"hp_mult": 1.1, "hit_mult": 1.2, "xp_mult": 1.2},  # aggressive
    "Deadlock Wraith": {"hp_mult": 1.0, "hit_mult": 0.9, "xp_mult": 0.9},  # balanced, low reward
    "Zombie Process":  {"hp_mult": 1.6, "hit_mult": 0.6, "xp_mult": 0.8},  # very tanky, weak hits
}

BASE_ENEMY_HP  = 80   # HP at level 1 before profile multiplier
...
def level_up(player_id: int):
    ...
    c.execute("""
        ...
        SET base_hp      = base_hp      + ?,
            max_hp       = max_hp       + ?,
            current_hp   = MAX(current_hp + ?, max_hp + ?),
            original_base_hit = original_base_hit + ?,
            base_hit     = base_hit     + ?,
            base_crit  = base_crit  + ?
        WHERE player_id = ?
    """, (hp_increase, hp_increase, hp_increase, hp_increase, hit_increase,
