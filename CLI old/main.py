...
def _handle_death(player_id):
    ...
    dsr, did_reset = db.record_death_and_check_reset(player_id)
    lives = db.get_lives(player_id)
...
def show_gear_sets_screen(player_id):
    ...
    all_sets   = db.get_all_gear_sets()
    unlocked   = {s["id"] for s in db.get_unlocked_gear_sets(player_id)}
    ...
    while True:
        ...
        if 1 <= choice <= len(unlocked_ids):
            ...
            db.ensure_starter_gear_in_db(chosen_set)
            ...
            if db.c.fetchone()[0] == 0:
                db.add_item(player_id, chosen_set["weapon_name"], 1)
            ...
            if db.c.fetchone()[0] == 0:
                db.add_item(player_id, chosen_set["armor_name"], 1)
...
def _handle_run_clear(player_id):
    ...
    """
    total_clears = db.increment_clears(player_id)
    ...
    if total_clears % 5 == 0:
        ...
        db.clear_user(player_id)

        enable_cbreak()
        clear_screen()
        typewrite(f"great work {username}...", delay=0.5)
        if skip(): disable_cbreak(); return total_clears
        typewrite("...", delay=0.5)
        if skip(): disable_cbreak(); return total_clears
        typewrite("..hm..", delay=0.5)
        if skip(): disable_cbreak(); return total_clears
        typewrite("there's no need for you now..", delay=0.4)
        if skip(): disable_cbreak(); return total_clears
        print()
        typewrite(f"DELETE data FROM users WHERE name = {username}")
        if skip(): disable_cbreak(); return total_clears
        typewrite("1..2..3..done!", delay=0.6)
        disable_cbreak()
    ...
    newly_unlocked = []
    if total_clears == 5:
        db.unlock_gear_set(player_id, 1)
        newly_unlocked.append(1)
    elif total_clears == 10:
        db.unlock_gear_set(player_id, 2)
        newly_unlocked.append(2)
    elif total_clears == 15:
        db.unlock_gear_set(player_id, 3)
...
def show_stats(player_id):
    ...
    lives = db.get_lives(player_id)
    life_icons = "♥ " * lives + "♡ " * (3 - lives)
    clears = db.get_total_clears(player_id)
...
def draw_combat_screen(player_id, enemy_id, events, active_defense, log, combo = 0):
    ...
    statuses = db.get_statuses(player_id)
    if statuses:
...
def player_attack(player_id, enemy_id):
    db.c.execute("SELECT base_hit, bonus_hit, base_crit, bonus_crit FROM player_stats WHERE player_id = ?", (player_id,))
    pstats = db.c.fetchone()
    ...
    db.conn.commit()
    db.c.execute("SELECT equipped_weapon FROM players WHERE id = ?", (player_id,))
    weapon_name = db.c.fetchone()["equipped_weapon"]
    element_bonus = False
    if weapon_name:
        db.c.execute("SELECT element FROM weapons WHERE name = ?", (weapon_name,))
        w_row = db.c.fetchone()
        db.c.execute("SELECT type FROM enemies WHERE id = ?", (enemy_id,))
        enemy_type = db.c.fetchone()["type"]
        # Strip OVERFLOW boss prefix
        base_type = enemy_type.replace("OVERFLOW — ", "").strip()
        enemy_element = db.ENEMY_ELEMENT.get(base_type)
        weapon_element = w_row["element"] if w_row else None
        if weapon_element and enemy_element and db.ELEMENT_WEAKNESS.get(weapon_element) == enemy_element:
            dmg = int(dmg * 1.5)
            element_bonus = True
...
def enemy_turn(player_id, enemy_id, events, active_defense):
    ...
    if random.random() < 0.15:
        ...
        db.apply_status(player_id, effect, duration)
...
def run_combat(player_id, enemy_id, events, active_defense=0, run_id=None):
    ...
    while True:
        ...
        status_log = db.tick_statuses(player_id)
        if status_log:
            log += status_log
        ...
        if any(s["effect"] == "DEADLOCK" for s in statuses):
            ...
            if db.c.fetchone()[0] <= 0:
                ...
                db.c.execute("UPDATE player_stats SET current_hp = max_hp WHERE player_id = ?", (player_id,))
                db.c.execute("DELETE FROM status_effects WHERE player_id = ?", (player_id,))
                db.conn.commit()
        ...
        if action == 1:
            ...
            if db.c.fetchone()[0] <= 0:
                ...
                if run_id:
                    ...
                    db.record_run_bytes(run_id, bytes_drop)
            ...
            db.c.execute("SELECT current_hp FROM player_stats WHERE player_id = ?", (player_id,))
            hp_before = db.c.fetchone()[0]

            active_defense, enemy_log = enemy_turn(player_id, enemy_id, events, active_defense)
            log += enemy_log

            db.c.execute("SELECT current_hp FROM player_stats WHERE player_id = ?", (player_id,))
            hp_after = db.c.fetchone()[0]
            ...
            if hp_after < hp_before:
                ...
                if combo >= 3:
                    ...
                    db.c.execute("UPDATE enemies SET base_hp = base_hp - ? WHERE id = ?", (chain_dmg, enemy_id))
                    db.conn.commit()
...
def show_npc(player_id, events):
    db.c.execute("SELECT kills, deaths FROM players WHERE id = ?", (player_id,))
    row = db.c.fetchone()
    kills  = row["kills"]
    deaths = row["deaths"]
...
def print_run_stats(player_id):
    data = db.fetch_runs_stats(player_id)
...
def run_dungeon(player_id, node_name, enc_type, events, run_id=None):
    ...
    for room_num in range(1, total_rooms + 1):
        ...
        # ---- TRAP ROOM ---- #
        if room_type == "trap":
            db.c.execute("SELECT base_crit + bonus_crit AS crit FROM player_stats WHERE player_id = ?",
                         (player_id,))
            crit = db.c.fetchone()["crit"]
    ...
    db.c.execute("SELECT level FROM players WHERE id = ?", (player_id,))
    player_level = db.c.fetchone()["level"]
    _, loot_name, _ = db.generate_gear(player_level)
    db.add_item(player_id, loot_name, 1)
    db.conn.commit()
...
def run_shop(player_id, node_name, events, path_id=None, run_seed=None):
    ...
    saved = db.load_shop_stock(path_id) if path_id else None

    if saved:
        # Reconstruct gear_stock and potion_stock from saved rows
        potion_names = {p[0] for p in db.get_potion_pool()}
        potion_pool_map = {p[0]: p for p in db.get_potion_pool()}
        gear_stock   = []
        potion_stock = []
        weapon_pool  = []
        armor_pool   = []
        ...
        for _ in range(SHOP_STOCK_SIZE * 2):
            _, name, _ = db.generate_gear(player_level, "weapon")
            db.c.execute("SELECT * FROM weapons WHERE name = ?", (name,))
            row = db.c.fetchone()
            if row is not None:
                weapon_pool.append(row)
        for _ in range(SHOP_STOCK_SIZE * 2):
            _, name, _ = db.generate_gear(player_level, "armor")
            db.c.execute("SELECT * FROM armors WHERE name = ?", (name,))
            row = db.c.fetchone()
            if row is not None:
                armor_pool.append(row)
        ...
        if not weapon_pool:
            db.c.execute("SELECT * FROM weapons ORDER BY RANDOM() LIMIT ?", (SHOP_STOCK_SIZE * 2,))
            weapon_pool = list(db.c.fetchall())
        if not armor_pool:
            db.c.execute("SELECT * FROM armors ORDER BY RANDOM() LIMIT ?", (SHOP_STOCK_SIZE * 2,))
        ...
        if path_id:
            ...
            db.save_shop_stock(path_id, stock_to_save)
    ...
    while in_shop:
        ...
        db.c.execute("SELECT bytes FROM player_stats WHERE player_id = ?", (player_id,))
        bytes = db.c.fetchone()["bytes"]
        ...
        if choice == 0:
            ...
            if confirm == 1:
                if bytes < price:
                    ...
                    db.c.execute("UPDATE player_stats SET bytes = bytes - ? WHERE player_id = ?",
                                 (price, player_id))
            ...
            if confirm == 1:
                if bytes < pprice:
                    ...
                    if path_id:
                        db.remove_shop_stock_item(path_id, pot[0])
            ...
            if confirm == 1:
                ...
                if inv_row["amount"] <= 1:
                    db.remove_gear_item_by_rowid(player_id, inv_row["rowid"], inv_row["item"])
                else:
                    db.c.execute(
                        "UPDATE inventory SET amount = amount - 1 WHERE rowid = ?",
                        (inv_row["rowid"],)
...
def apply_solar_eclipse(player_id, events, remove=False):
    ...
    if class_name != "The Indexer":
        return
    if remove:
        db.c.execute("UPDATE player_stats SET bonus_crit = bonus_crit / 2 WHERE player_id = ?",
                     (player_id,))
    else:
        db.c.execute("UPDATE player_stats SET bonus_crit = bonus_crit * 2 WHERE player_id = ?",
                     (player_id,))
    ...
    while True:
        ...
        if action == "1":
            ...
            if item_name in potion_names:
                result = db.apply_potion(player_id, item_name)
            ...
            confirm = input("> ").strip()
            if confirm == "1":
                db.remove_gear_item_by_rowid(player_id, selected_item["rowid"], item_name)
...
def apply_cheat(player_id, code):
    ...
    if cheat["bytes"]:
        db.c.execute("UPDATE player_stats SET bytes = bytes + ? WHERE player_id = ?", (cheat["bytes"], player_id))
        typewrite(f"  +{cheat['bytes']} bytes", delay=0.01)

    if cheat["hp"]:
        db.c.execute("UPDATE player_stats SET current_hp = MIN(current_hp + ?, max_hp) WHERE player_id = ?", (cheat["hp"], player_id))
        typewrite("  HP restored", delay=0.01)

    if cheat["xp"]:
        db.c.execute("UPDATE players SET experience = experience + ? WHERE id = ?", (cheat["xp"], player_id))
        typewrite(f"  +{cheat['xp']} xp", delay=0.01)
        db.c.execute("SELECT level, experience FROM players WHERE id = ?", (player_id,))
        row = db.c.fetchone()
        while row["experience"] >= db.experience_needed_for_next_level(row["level"]):
            db.level_up(player_id)
            db.c.execute("SELECT level, experience FROM players WHERE id = ?", (player_id,))
            row = db.c.fetchone()

    for item_name, amount in cheat["items"]:
        db.add_item(player_id, item_name, amount)
...
try:
    ...
    while run:
        ...
        while play:
            clear_screen()
            unlocked_sets = db.get_unlocked_gear_sets(player_id)
            total_clears  = db.get_total_clears(player_id)
            lives         = db.get_lives(player_id)
            life_icons    = "♥ " * lives + "♡ " * (3 - lives)
        ...
        while adventuring:
            ...
            while path_running:
                ...
                flavour = db.get_node_flavour(enc_type)
                if flavour:
                    print()
                ...
                if enc_type == -1:
                    ...
                    # Check for ending
                    overflow_kills = db.record_overflow_kill(player_id)
                    if overflow_kills >= db.OVERFLOW_BOSSES_TOTAL:
